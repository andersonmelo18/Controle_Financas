// js/cartoes.js
// VERSÃO 5.3 (Corrigido: Bugs no modal 'Reverter' e 'currentTarget')
...
// v5.3: Removemos a referência global, vamos buscar por ID
...
// ===============================================================
// FASE 2A: GERENCIADOR DE CARTÕES (v5.2)
// ===============================================================
...
function renderizarFaturas(estadoGastos) {
    ...
    // 5. Atualizar a UI (v5.1: Adiciona cores de status)
    Object.values(meusCartoes).forEach(cartao => {
...
// ===============================================================
// FASE 3B: LÓGICA DE PAGAMENTO (v5.1)
// ===============================================================
...
// ===============================================================
// CORREÇÃO (v5.3): Lógica do modal de Reversão - CORRIGIDA PARA REMOVER DE AMBOS OS NÓS
// ===============================================================
// cartoes.js (Função handleReverterPagamentoClick - CORRIGIDA para resgatar o valor)

async function handleReverterPagamentoClick(cartao, valorInicial) {
    // ATENÇÃO: O 'valorInicial' (vindo do botão) é provavelmente zero.
    // Usaremos este valor apenas no modal.
    modalReverterMessage.textContent = `Tem certeza que quer reverter o pagamento da fatura ${cartao.nome} (${formatCurrency(valorInicial)})? O valor será devolvido ao seu Saldo em Caixa.`;
    ...
    const reverterFn = async () => {
        ...
        let valorFaturaPago = 0; // Váriavel para armazenar o valor CORRETO do DB.
        
        try {
            // 1. Acha e remove o registro de pagamento das PENDÊNCIAS
            const snapshotPendencias = await get(ref(db, pathPendencias));
            let pagamentoIdPendencia = null;
            if (snapshotPendencias.exists()) {
                snapshotPendencias.forEach(child => {
                    const pendencia = child.val();
                    if (pendencia.descricao === `Pagamento Fatura ${cartao.nome}` && pendencia.status === 'pago') {
                        pagamentoIdPendencia = pendencia.id;
                        // ✅ CORREÇÃO 1: Captura o valor real do DB
                        valorFaturaPago = pendencia.valor; 
                    }
                });
            }

            if (!pagamentoIdPendencia) {
                console.warn("Registo de pagamento não encontrado em 'pendencias'. Tentando buscar em 'despesas'.");
            } else {
                await remove(ref(db, `${pathPendencias}/${pagamentoIdPendencia}`));
            }

            // 2. Acha e remove o registro de pagamento das DESPESAS
            const snapshotDespesas = await get(ref(db, pathDespesas));
            let pagamentoIdDespesa = null;
            if (snapshotDespesas.exists()) {
                snapshotDespesas.forEach(child => {
                    const despesa = child.val();
                    if (despesa.descricao === `Pagamento Fatura ${cartao.nome}` && despesa.categoria === 'Contas') { 
                        pagamentoIdDespesa = despesa.id;
                        // ✅ CORREÇÃO 2: Se o valor não foi pego nas pendências, pega nas despesas
                        if (valorFaturaPago === 0) {
                            valorFaturaPago = despesa.valor;
                        }
                    }
                });
            }
            
            if (pagamentoIdDespesa) {
                await remove(ref(db, `${pathDespesas}/${pagamentoIdDespesa}`));
            } else {
                 console.warn("Registo de despesa de pagamento não encontrado em 'despesas'.");
            }

            // 3. Reverte o saldo (usando o valor correto resgatado do DB)
            if (valorFaturaPago > 0) {
                await updateSaldoGlobal(valorFaturaPago); // Usa o valor corrigido!
            } else {
                alert("Erro: Não foi possível determinar o valor pago para reverter o saldo. Nenhuma alteração foi feita no saldo.");
                console.error("Valor pago para reversão é zero ou negativo:", valorFaturaPago);
            }
    ...
    // ... (restante do código do modal, sem alterações)
    ...
    const newBtnConfirm = currentConfirmBtn.cloneNode(true);
    const newBtnCancel = currentCancelBtn.cloneNode(true);

    currentConfirmBtn.parentNode.replaceChild(newBtnConfirm, currentConfirmBtn);
    currentCancelBtn.parentNode.replaceChild(newBtnCancel, currentCancelBtn);
...
// ===============================================================
// FASE 3B: LÓGICA DE PAGAMENTO (v5.1 - CORRIGIDA)
// ===============================================================

// ... (Função handlePagarFaturaClick permanece a mesma - Ela apenas chama esta função e o refresh) ...

async function registrarPagamentoFatura(cartao, valor) {
    // 1. Paths para registro
    const pathDespesas = `dados/${userId}/despesas/${currentYear}-${currentMonth}`; // PARA O ORGANIZADOR DE DESPESAS
    const pathPendencias = `dados/${userId}/pendencias/${currentYear}-${currentMonth}`; // PARA O STATUS DE PAGO DA FATURA
    ...
    // Objeto para a lista de DESPESAS (Formato corrigido)
    const despesa = {
        data: dataPagamento, // CORRIGIDO: Usa 'data' para ordenação na tabela de despesas
        categoria: 'Contas', // NOVO: Adiciona a categoria para o despesas.js
        ...
        comprovante: null // NOVO: Adiciona campo comprovante
    };
    
    // Objeto para o status em PENDÊNCIAS (Formato antigo necessário para o status check)
    const pendencia = {
    ...
    try {
        // 1. REGISTRA A TRANSAÇÃO NA LISTA DE DESPESAS (para aparecer organizada)
        const newRefDespesas = push(ref(db, pathDespesas));
        await set(newRefDespesas, { ...despesa, id: newRefDespesas.key });

        // 2. REGISTRA A TRANSAÇÃO NAS PENDÊNCIAS (para que a fatura seja marcada como PAGA)
        const newRefPendencias = push(ref(db, pathPendencias));
        await set(newRefPendencias, { ...pendencia, id: newRefPendencias.key });
